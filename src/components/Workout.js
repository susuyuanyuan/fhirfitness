...
const Workout = () => {
    ...
    useEffect(() => {
        let interval = null;
        if (isActive && timer > 0) {
            interval = setInterval(function () { setTimer(timer - 1) }, 1000);
        } else if (timer === 0) {
    ...
    const renderTimer = task => {
        ...
        return (
            <div>
                ...
                <div>
                    {counting === null ? <button className="timer" onClick={() => handleStart(task)}>Start</button> : null}
                    {counting === task ? <button className="timer" onClick={() => reset()}>Retake Challenge</button> : null}
                </div>
            </div>
        );
    };
